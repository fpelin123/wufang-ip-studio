...
function fallbackDocument(project: GenerateRequest["project"], step?: GenerateRequest["step"]) {
...
export async function POST(request: Request) {
  ...
  const { project, provider, step } = body
  ...
  if (!provider?.apiKey || !provider.baseUrl || !provider.textModel) {
    return NextResponse.json({
      content: fallbackDocument(project, step),
  ...
  return NextResponse.json({
    content: typeof content === "string" ? content : fallbackDocument(project, step),
    source: "model",
  })
}
